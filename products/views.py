    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if basket_form.is_valid():
                ...
                existing_basket_item = Basket.objects.filter(
                ...
                if existing_basket_item:
                    ...
                    messages.success(
                        request, f'The quantity of {product.title} has been '
                        f'updated in your basket!')
                else:
                    ...
                    order.save()
                    messages.success(
                        request, f'{product.title} added successfully '
                        f'to your basket!')

                return redirect('basket')
            else:
                messages.error(
                    request, 'There was an error adding the product to '
                    f'Your basket. Please try again.')
...
def remove_item(request, basket_id):
    ...
    try:
        basket_item = get_object_or_404(
            Basket, id=basket_id, user=request.user)
        basket_item.delete()
        messages.add_message(
            request, messages.ERROR, 'Item removed from your basket!')

        return HttpResponseRedirect(reverse('basket'))
    except Exception as e:
        messages.error(
            request, f'Failed to remove item because of the following '
            f'error:\n{str(e)}')
        return redirect('basket')
...
def after_payment(request):
    ...
    ***Template***
    ...
    try:
        ...
        basket_items.delete()
        messages.success(
            request, 'Payment processed successfully. Your basket has been '
            f'cleared.')
        ...
        messages.error(
            request, f'Payment processing failed because of the following '
            f'error:\n{str(e)}')
        return render(request, '500.html', status=500)
...
def add_to_favorites(request, product_id):
    ...
    try:
        ...
        messages.error(
            request, f'The following unexpected error occurred:\n{str(e)}')
        return redirect('products')
...
def remove_favorite(request, favorite_id):
    ...
    try:
        favorite = get_object_or_404(
            Favorite, id=favorite_id, user=request.user)
        favorite.delete()
